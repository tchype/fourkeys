...
github_pipeline(){
  ...
  gcloud run services --platform managed add-iam-policy-binding github-worker \
  ...
  export PUSH_ENDPOINT_URL=$(gcloud run services --platform managed --region ${FOURKEYS_REGION} describe github-worker --format=yaml | grep url | head -1 | sed -e 's/  *url: //g')
...
gitlab_pipeline(){
  ...
  gcloud run services --platform managed add-iam-policy-binding gitlab-worker \
   --member="serviceAccount:cloud-run-pubsub-invoker@${FOURKEYS_PROJECT}.iam.gserviceaccount.com" \
   --role=roles/run.invoker

  # Get push endpoint for gitlab-worker
  export PUSH_ENDPOINT_URL=$(gcloud run services --platform managed --region ${FOURKEYS_REGION} describe gitlab-worker --format=yaml | grep url | head -1 | sed -e 's/  *url: //g')
  # configure the subscription push identity
...
cloud_build_pipeline(){
  ...
  gcloud run services --platform managed add-iam-policy-binding cloud-build-worker \
   --member="serviceAccount:cloud-run-pubsub-invoker@${FOURKEYS_PROJECT}.iam.gserviceaccount.com" \
   --role=roles/run.invoker

  # Get push endpoint for cloud-build-worker
  export PUSH_ENDPOINT_URL=$(gcloud run services --platform managed --region ${FOURKEYS_REGION} describe cloud-build-worker --format=yaml | grep url | head -1 | sed -e 's/  *url: //g')
  # configure the subscription push identity
...
tekton_pipeline(){
  ...
  gcloud run services --platform managed add-iam-policy-binding tekton-worker \
   --member="serviceAccount:cloud-run-pubsub-invoker@${FOURKEYS_PROJECT}.iam.gserviceaccount.com" \
   --role=roles/run.invoker

  # Get push endpoint for github-worker
  export PUSH_ENDPOINT_URL=$(gcloud run services --platform managed --region ${FOURKEYS_REGION} describe tekton-worker --format=yaml | grep url | head -1 | sed -e 's/  *url: //g')
  # configure the subscription push identity
...
generate_data(){
  ...
  export WEBHOOK=$(gcloud run services --platform managed --region ${FOURKEYS_REGION} describe event-handler --format=yaml | grep url | head -1 | sed -e 's/  *url: //g')
